...
double f0(double x) { return 12 * x + 11;
}
int main() {
...
printf("Enter initial approximation:: "); scanf("%lf", &xn);
xn0 = xn;
printf("x_i\t| f(x_i)\n"); 
while (1) 
{
if (fabs(f0(xn0) < slope_lim))  {
printf("Slope is too small. Stoping execution...."); return 0;
}
xn = xn0 - (f(xn0) / f0(xn0)); printf("%lf\t| %lf\n", xn, f(xn)); if (f(xn) == 0) {
printf("Result :: %lf", xn);
break;
